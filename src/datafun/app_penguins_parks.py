...
def make_plots(
    ...
    # === NEW PLOT: Flipper length vs Bill length ===

    LOG.info("------ Creating Flipper vs Bill Length Plot -----------")

    plt.figure()

    flipper_bill_plot: Axes = sns.regplot(
        x=df_model["flipper_length_mm"], y=df_model["bill_length_mm"]
    )

    flipper_bill_plot.set_xlabel("Flipper length (mm)")
    flipper_bill_plot.set_ylabel("Bill length (mm)")
    flipper_bill_plot.set_title("Flipper length vs Bill length")
